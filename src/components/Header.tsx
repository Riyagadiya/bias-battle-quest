  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ease-in-out ${
        scrolled ? "bg-white bg-opacity-90 backdrop-blur-sm shadow-sm py-2" : "bg-transparent py-3 md:py-4"
      }`}
    >
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between">
          ...
          >
            {mobileMenuOpen ? (
              <X size={20} className="text-black transition-all duration-300 ease-in-out" />
            ) : (
              <Menu size={20} className="text-black transition-all duration-300 ease-in-out" />
            )}
          </button>
        </div>
      </div>
